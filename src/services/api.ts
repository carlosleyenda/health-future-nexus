...
export class EmergencyService {
  static async sendAlert(alertData: {
    ...
    // Simular envío de alerta a servicios de emergencia
...
export class DevicesService {
  ...
  static async getDeviceData(deviceId: string, timeRange: string) {
    ...
    // Simular datos basados en el tipo de dispositivo
    const mockData = [
  ...
  static async getDeviceAlerts(patientId: string) {
    await delay(300);
    return [
      {
        id: 'alert1',
        deviceId: 'dev1',
        type: 'low_battery',
        message: 'Batería baja en Apple Watch',
        severity: 'warning',
        timestamp: new Date().toISOString()
      }
    ];
